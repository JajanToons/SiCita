...
import {
  ...
  // Gauge, // Jika ingin digunakan untuk ikon Kekeruhan, perlu diimpor
} from "lucide-react";
...
export default function DeviceHistoryPage() {
  ...
  // State baru untuk mode tampilan chart TDS & Kekeruhan
  ...
  const columnsToDisplay = useMemo(() => {
    return sensorDataColumnsDefinition.filter((column) => {
      // column.id is a valid property (string | undefined) on ColumnDef
      // For accessorKey, use a specific type assertion instead of any
      const columnId =
  ...
  if (
    ...
    return (
      <div className="container mx-auto p-4 md:p-6 space-y-6 animate-pulse">
        {/* ... (Skeleton UI halaman tetap sama) ... */}
        ...
        {[...Array(3)].map(
          (
            _,
            index // Ubah menjadi 3 chart skeleton utama
          ) => (
            <Card key={index}>
              <CardHeader>
  ...
  if (!deviceIdFromUrl && !initialLoading) {
    return (
      <div className="container mx-auto p-6 text-center min-h-screen flex flex-col justify-center items-center">
        {/* ... (Error Device ID tidak valid tetap sama) ... */}
  ...
  if (error && historicalData.length === 0 && !isLoading && !initialLoading) {
    return (
      <div className="container mx-auto p-6 text-center min-h-screen flex flex-col justify-center items-center">
        {/* ... (Error Gagal Memuat Riwayat tetap sama) ... */}
        <AlertTriangle className="mx-auto h-12 w-12 text-destructive mb-4" />
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 space-y-6">
      {" "}
      {/* Dihapus overflow-x-hidden dari sini jika ada, agar tidak konflik dengan overflow-x-auto di bawah */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
      ...
      <Card>
        {/* ... (CardHeader untuk Filter & Opsi Data tetap sama) ... */}
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-x-4 gap-y-2 border-b pb-3 pt-4 px-4">
      ...
      {/* --- CHART 1: KETINGGIAN AIR (Tidak berubah signifikan) --- */}
      <Card>
        {/* ... (Konten Chart Ketinggian Air tetap sama) ... */}
        <CardHeader>
        ...
        <CardContent className="p-0">
          {isLoading && chartData.length === 0 && !error ? (
            ...
            <div className="px-2 pt-4 sm:px-6 sm:pt-6">
              <ChartContainer
                config={waterLevelChartConfig as ShadCNChartConfig} // Cast to ShadCNChartConfig
      ...
      {/* --- CHART 2: SUHU & CURAH HUJAN (Tidak berubah signifikan) --- */}
      <Card>
        {/* ... (Konten Chart Suhu & Curah Hujan tetap sama) ... */}
        <CardHeader>
        ...
        <CardContent className="p-0">
          {isLoading && chartData.length === 0 && !error ? (
            ...
            <div className="px-2 pt-4 sm:px-6 sm:pt-6">
              <ChartContainer
                config={tempRainChartConfig as ShadCNChartConfig} // Cast to ShadCNChartConfig
                className="h-[250px] sm:h-[350px] w-full"
              >
                <AreaChart
                  data={chartData}
      ...
      {/* --- CHART 3: TDS & KEKERUHAN (MODIFIED) --- */}
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div>
              <CardTitle className="text-lg flex items-center">
                <TestTube2 // Atau ikon lain yang sesuai
                  className="mr-2 h-5 w-5"
                  style={{
        ...
        <CardContent className="p-0">
          {isLoading && chartData.length === 0 && !error ? (
            ...
            <div className="px-2 pt-4 sm:px-6 sm:pt-6">
              <ChartContainer
                config={tdsTurbidityChartConfig as ShadCNChartConfig} // Cast to ShadCNChartConfig
                className="h-[250px] sm:h-[350px] w-full"
              >
                <AreaChart
                  data={chartData}
                  margin={{
                    ...
                    right:
                      ...
                      tdsTurbidityChartConfig.turbidityNtu
                        ? 20 // Tambah margin kanan jika ada sumbu Y kedua untuk kekeruhan
                        : 10,
                    left: -20,
                    bottom: 20,
                  }}
                >
                  ...
                  {/* Sumbu Y untuk Kekeruhan */}
                  {tdsTurbidityMode === "turbidity_only" &&
                    tdsTurbidityChartConfig.turbidityNtu && (
                      <YAxis
                        yAxisId="left" // Gunakan 'left' jika hanya kekeruhan
                  ...
                  {(tdsTurbidityMode === "tds_only" ||
                    ...
                    tdsTurbidityChartConfig.tdsPpm && (
                      <Area
                        ...
                        connectNulls={true}
                        name={tdsTurbidityChartConfig.tdsPpm.label.replace(
                          " : ",
                          ""
                        )}
                      />
                    )}
                  {/* Area untuk Kekeruhan */}
                  {(tdsTurbidityMode === "turbidity_only" ||
                    tdsTurbidityMode === "both") &&
                    tdsTurbidityChartConfig.turbidityNtu && (
                      <Area
                        ...
                        connectNulls={true}
                        name={tdsTurbidityChartConfig.turbidityNtu.label.replace(
                          " : ",
                          ""
                        )}
                      />
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading && historicalData.length === 0 && !error ? (
            ...
            // MODIFIKASI DI SINI:
            // Pastikan div ini dapat di-scroll dan table di dalamnya bisa mengambil lebar minimum yang diperlukan.
            <div className="overflow-x-auto p-2 sm:p-4">
              {/* Anda mungkin perlu memastikan komponen DataTable itu sendiri atau tabel HTML di dalamnya
                  memiliki min-width jika kontennya tidak secara otomatis memaksa div ini untuk scroll.
                  Contoh: <DataTable className="min-w-[700px]" ... /> jika DataTable menerima className.
                  Namun, biasanya `overflow-x-auto` sudah cukup jika table di dalamnya proper. */}
